...
def cosine_similarity_over_time(x, recon, eps=1e-8):
    """
    Compute cosine distance (1 - cosine similarity) between original and reconstructed
    spectrograms along the time axis.

    Args:
        x: Tensor of shape [freq, time] (e.g., [1, 80, 400])
        recon: Tensor of same shape as x
        eps: small value to avoid division by zero in cosine similarity

    Returns:
        Tensor of shape [time] with cosine distance per time step
    """
    x = x.squeeze(0)      # [freq, time]
    recon = recon.squeeze(0)
    
    x_t = x.T  # [time, freq]
    recon_t = recon.T  # [time, freq]
    sim = F.cosine_similarity(x_t, recon_t, dim=1, eps=eps)  # [time]
    distance = 1 - sim  # cosine distance (0 = identical, 2 = opposite)
    return distance
...
def plot_fbank(fbank, title=None, cmap='inferno', compute_overlap=False, annotations=None, lossmap=None):
    
    if compute_overlap:
        if annotations is None:
            raise ValueError("annotations are required for plotting the human-annotated regions...")
        plt.figure(figsize=(8, 4))
        plt.imshow(fbank, aspect='auto', origin='lower', cmap='inferno') # fbank here is the input spectrogram to evaluate
        plt.title("Input Spectrogram with Human Annotations")
        # Overlay annotations
        for i, (start, end) in enumerate(annotations):
            if i == 0:
                plt.axvspan(start, end, color='blue', alpha=0.3, label='Human Annotation')
            else:
        ...
        plt.figure(figsize=(8, 4))
        plt.imshow(lossmap, aspect='auto', origin='lower', cmap='inferno') # fbank here is the input spectrogram to evaluate
        plt.title("Reconstruction Error Map with Human Annotations")
        # Overlay annotations
        for i, (start, end) in enumerate(annotations):
            if i == 0:
                plt.axvspan(start, end, color='blue', alpha=0.3, label='Human Annotation')
...
def normalize_and_find_peaks(curve, percentile=95, distance=5):
    """Normalize curve and find peaks above a threshold percentile."""
    norm_curve = (curve - curve.mean()) / curve.std()
    norm_curve_np = norm_curve.cpu().numpy()
    threshold = np.percentile(norm_curve_np, percentile)
    peaks, _ = find_peaks(norm_curve_np, height=threshold, distance=distance)
    # manually check start
    if norm_curve[0] > norm_curve[1] and norm_curve[0] > threshold:
        peaks = np.insert(peaks, 0, 0)
    # manually check end
    if norm_curve[-1] > norm_curve[-2] and norm_curve[-1] > threshold:
        peaks = np.append(peaks, len(norm_curve) - 1)
    return norm_curve_np, peaks, threshold
...
class ModelTrainer(Model):
    ...
    def evaluate_region_level(self, lossmap, peaks_percentile=98, peaks_distance=3):
        """
        Computes region-level anomaly score curve and peak detection.
    
        Returns:
            norm_curve (np.ndarray): Normalized 1D error curve over time
            peaks (np.ndarray): Indices of detected peaks
        """
        # lossmap --> shape: [1, F, T]
        curve = compute_error_curve(lossmap, axis=1) # sum over frequency
        norm_curve, peaks, _ = normalize_and_find_peaks(curve, percentile=peaks_percentile, distance=peaks_distance)
        return norm_curve, peaks

    def evaluate_overlap_metric(self, binary_mask, annotations, peaks_percentile=98, peaks_distance=3):
        """
        Compute the percentage of detected anomaly peaks that fall inside annotated regions.
    
        Returns:
            peak_overlap_score (float): [0, 1]
        """
        ...
        peaks_inside = sum(any(start <= p < end for (start, end) in annotations) for p in peaks)
        return peaks_inside / len(peaks)
    ...
    def get_loss(self, x, masked_x=None, ae_mask=None, #positions=None, patches=None, patchmasks=None, 
                 ...
                 compute_overlap=False, annotations=None,
                 cosine_similarity=False,
                 artificial_signal_params=None,
                 layer_attribution=False,
                 overlap_metric=False,
                 region_level_eval=False, return_2d_error_map=False,
                ):
        # x --> original_input
        ...
        # if plot or threshold_2d_error_map:
        ...
        if threshold_2d_error_map: # 2D thresholding
            threshold = torch.quantile(lossmap, percentile_value)
            binary_mask = (lossmap > threshold).to(torch.uint8)
            if plot:
                plot_fbank(binary_mask[0], f"Error Regions (Binary Mask > {percentile_value*100}th Percentile)")

        # Return normalized error-over-time for region-level evaluation
        if region_level_eval:
            norm_curve, peaks = self.evaluate_region_level(lossmap, peaks_percentile, peaks_distance)
            return norm_curve, peaks  # 1D numpy array

        if compute_overlap: # compute overlap between high-error regions and human-annotated regions
            if annotations is None or not threshold_2d_error_map:
                raise ValueError("annotations and threshold_2d_error_map are required...")
                
            annotation_mask = torch.zeros_like(lossmap[0]).to(torch.uint8)
            for start, end in annotations:
                annotation_mask[:, start:end] = 1  # assume full frequency range

            if plot:
                plot_fbank(annotation_mask, title='annotation mask')
                # print('annotation_mask, binary_mask', annotation_mask.shape, binary_mask.shape, annotation_mask.dtype, binary_mask.dtype) 
                # plot the annotation on top of the input non-masked spectrogram
                plot_fbank(x[0], compute_overlap=compute_overlap, annotations=annotations, lossmap=binary_mask[0])
            
            # === Error Over Time ===
            process_error_analysis(
                binary_mask=binary_mask,
                axis=1,  # sum over frequency
                peaks_percentile=peaks_percentile,
                peaks_distance=peaks_distance,
                axis_label='Time',
                plot=plot,
            )
        
            # === Error Over Frequency ===
            process_error_analysis(
                binary_mask=binary_mask,
                axis=2,  # sum over time
                peaks_percentile=peaks_percentile,
                peaks_distance=peaks_distance,
                axis_label='Frequency',
                plot=plot,
            )
        if compute_overlap and overlap_metric:
            peak_overlap_score = self.evaluate_overlap_metric(binary_mask, annotations, peaks_percentile, peaks_distance)
            return peak_overlap_score
        ...
        if cosine_similarity:
            cos_loss = cosine_similarity_over_time(x, prediction)
            plot_cosine_distance_over_time(cos_loss, title="Reconstruction Cosine Distance Over Time")
